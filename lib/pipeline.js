...
function pipelineFactory(basePath, logger, vinylStream) {
    ...
    return function (files, added, changed) {
        ...
        // Create a copy of the original files array to prevent changes from tainting Karma's file cache.
        files = {
            served: files.served.slice(),
            included: files.included.slice()
        };

        d.on('error', function (e) {
            log.error('Error caught in vinyl stream: ' + (e.stack ? e.stack : e));
        ...
        d.run(function () {
            try {
                ...
                log.error('Error caught in config function: ' + (e.stack ? e.stack : e));
            }
        });
    ...
    function synchronizeFiles(karmaFiles, inputPaths, outputFiles) {
        _.each(inputPaths, function (path) {
            ...
            if (replacement) {
                ...
                var replacementKarmaFile = createKarmaFile(replacement);
                karmaFiles.served.splice(servedIdx, 1, replacementKarmaFile);
                if (includedIdx > -1) {
                    karmaFiles.included.splice(servedIdx, 1, replacementKarmaFile);
                }
            }
            else {
                ...
                if (includedIdx > -1) {
                    karmaFiles.included.splice(includedIdx, 1);
                }
            }
        });
    ...
    function createVinylFile(karmaFile) {
        return new File({
            ...
            base: basePath,
            path: karmaFile.path,
            contents: new Buffer(karmaFile.content || ''),
            karmaProps: {
