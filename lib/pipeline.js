...
function pipelineFactory(basePath, logger, vinylStream) {
    ...
    function synchronizeFiles(karmaFiles, inputPaths, outputFiles) {
        _.each(inputPaths, function (path) {
            ...
            if (replacement) {
                ...
                replaceKarmaFile(karmaFiles.served[servedIdx], replacement);
            }
            else {
    ...
    function replaceKarmaFile(karmaFile, vinylFile) {
        var content = vinylFile.contents.toString();
        karmaFile.path = vinylFile.path;
        karmaFile.contentPath = null;
        karmaFile.content = content;
        karmaFile.sha = sha1(content);
    }
